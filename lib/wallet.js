...
Wallet.prototype.fetchAllOrganizationList = function() {
  ...
  .then(data => {
    ...
    for(var i in keys){
      var key = Object.keys(orgs[i])[0];
      var OrgGroup = {
        "Groupname": key,
        "organizations": [],
      }
      var group = orgs[i][key];
      for(var j = 0; j < group.length; j++){
        
        var org = {
          "name": group[j]["Organization name: "],
          "address": group[j]["Organization address: "],
        }
        OrgGroup["organizations"].push(org);
      }
      newOrgs.push(OrgGroup);
    
    }
    console.log("NewOrgs");
    console.log(newOrgs);
    this.allOrgLists = newOrgs;
  })
  .catch(console.error);
}
